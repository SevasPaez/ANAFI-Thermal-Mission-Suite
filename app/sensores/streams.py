...
class SensorStream:
    """Hilo que actualiza un snapshot de sensores de forma continua.
    Lee siempre el último dato disponible y lo expone como propiedad thread-safe.
    """
    ...
    def _loop(self):
        ...
        while self._running:
            try:
                ...
                # Mantener el hilo vivo aunque falle una lectura puntual
                pass
            time.sleep(period)
...
class FrameStream:
    """Lector de frames por hilo dedicado que mantiene el frame más reciente.
    Intenta con FFMPEG -> default backend -> fallback MJPEG (HTTP multipart).
    """
    ...
    def start(self):
        ...
        # Intento 3: fallback MJPEG por HTTP multipart
        ...
        # Si llegamos aquí, no fue posible abrir
        self._running = False
            ...
            with urllib.request.urlopen(req, timeout=5) as resp:
                ...
                while self._running:
                    ...
                    if not chunk:
                        ...
                        continue
                    buf += chunk
                    # Buscar JPEG por marcadores (SOI/EOI)
                    while True:
