...
def _is_radiometric_jpg(path: str) -> bool:
    """Heurística rápida: los JPG térmicos radiométricos de ANAFI Thermal
    suelen incluir un PNG 16-bit embebido (firma PNG dentro del archivo).
    """
...
def _pick_radiometric_file(download_dir: str) -> str:
    """Elige el JPG (o JPEG) descargado que contenga radiometría.

    Olympe puede descargar varios recursos (JPG, DNG, thumbnails, etc.).
    Aquí preferimos el que tenga firma PARROT + PNG embebido.
    """
    ...
    # mayor tamaño suele ser el recurso principal
    candidates.sort(key=lambda t: t[1], reverse=True)
    return candidates[0][0]


def _score_dng_candidate(path: str) -> float:
    """Score para escoger el mejor sidecar radiométrico.

    En algunos firmwares del ANAFI Thermal, el archivo con extensión .DNG puede
    ser realmente un contenedor JPEG que incluye un PNG 16-bit embebido.

    Olympe también puede descargar más de un "dng" o recursos extra.
    Aquí intentamos escoger el candidato que más probablemente tenga el payload
    radiométrico.
    """
    ...
    score = float(sz) / 1024.0  # base por tamaño

    try:
        # Leer una ventana razonable (la firma PNG suele aparecer temprano, pero no siempre).
        with open(path, "rb") as f:
            head = f.read(512 * 1024)  # 512 KB
            if sz > 512 * 1024:
...
def _pick_best_radiometric_dng(download_dir: str) -> str:
    """Escoge el mejor candidato .DNG dentro de download_dir."""
...
def take_thermal_photo(drone=None, capture_root="."):
    ...
    # Preserve current thermal mode (so RGB live view can stay RGB)
    try:
    ...
    # Importante: esperar a que termine la descarga para que no queden futures
    # corriendo cuando se encadena una misión tras otra (evita warnings de cleanup).
    ...
    # En lugar de depender de HTTP (que puede no ser accesible con SkyController),
    # tomamos el archivo que Olympe ya descargó en drone.media.download_dir.
    from sensores.media_utils import copy_as, pick_best_downloaded_file

    # 1) Guardar un JPG para visualización
    try:
        best = _pick_radiometric_file(drone.media.download_dir)
    except Exception:
        # fallback: escoger el JPG más grande
    ...
    # 2) Guardar también el DNG si existe.
    #    Ojo: hay casos donde el "DNG" que más pesa no es el radiométrico.
    try:
    ...
    # Generar y cachear matrices térmicas (RAW/TempC) para hover/segmentación.
    # Si falla la extracción radiométrica, no rompemos la captura.
    try:
