"""Extracción y persistencia de matrices térmicas por píxel.

Esta versión prioriza el mismo camino que usa FreeFlight/FLIR en las fotos del
ANAFI Thermal que has compartido:

1) Bloque APP1 FLIR -> record RawData (PNG 16-bit)
2) Corrección de byte order indicada por RawDataByteOrder
3) Conversión radiométrica con CameraInfo (Planck + ventana IR + humedad)

El bloque APP1 `PART` se deja como *fallback* porque en estas capturas resulta
coherente visualmente, pero no reproduce bien la temperatura absoluta de fondo.
"""
...
def _compute_matrices(jpg_path: str) -> Tuple[np.ndarray, Optional[np.ndarray], Dict[str, Any]]:
    ...
    # Fallback 2: PNG16 embebido sin calibración. Solo guardamos RAW.
    for cand in candidates:
