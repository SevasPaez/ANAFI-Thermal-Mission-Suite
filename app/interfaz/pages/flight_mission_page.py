# interfaz/pages/flight_mission_page.py
...
# Carpeta de guardado (si no existe MEDIA_ROOT, usar ./interfaz/media)
try:
    from config import MEDIA_ROOT  # opcional
...
BG_PRIMARY = "#161b22"  # azul
BG_PANEL = "#161b22"
FG_PRIMARY = "white"

class FlightMissionPage(tk.Frame):
    """Editor + ejecutor básico de misiones basadas en JSON.

    - Crea/edita waypoints (dx, dy, dz, dyaw, wait_s)
    - Define acción simple por waypoint (foto/video)
    - Exporta/Importa JSON
    - Ejecuta tipo: moveby_waypoints (Olympe moveBy)
    """
    ...
    def __init__(self, parent, drone=None, controller=None):
        ...
        # ----------- Photo metadata (CSV/XLSX) -----------
        # Guardamos metadatos de fotos tomadas durante una ejecución de misión.
        ...
        # ----------- Failsafe -----------
        # Nota: si la misión falla (pérdida de conexión, moveBy rechazado, etc.),
        # se intentará realizar un LAND inmediato (sin Return-To-Home).

        # Cache: some Olympe builds don't ship SpeedSettings/MaxHorizontalSpeed.
        # If we probe it on every run it spams olympe.module_loader errors.
        self._speed_cmd_checked = False
        self._speed_cmd_cls = None

        # ----------- State (Waypoint editor) -----------
        self.wp_dx = tk.StringVar(value="0.0")
        self.wp_dy = tk.StringVar(value="0.0")
        self.wp_dz = tk.StringVar(value="0.0")
        self.wp_dyaw = tk.StringVar(value="0.0")  # degrees
    ...
    def on_show(self):
        # no-op, pero dejamos el hook para futuro
        pass
    ...
    def _default_metadata_basename(self) -> str:
        # El usuario pidió que el nombre incluya "genai".
    ...
    def _export_metadata_dialog(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            # Excel abre siempre en celdas. Si el usuario elige CSV,
            # el exportador usa ';' + UTF-8 BOM para compatibilidad.
    ...
    def _prompt_export_after_mission(self):
        # Se llama desde el hilo UI al terminar/cancelar.
        if not self.meta_store.records:
            return
        try:
            ...
            # No bloquear si el diálogo no puede abrirse
            return
    ...
    def _ensure_drone_for_control(self):
        """Reusa una conexión Olympe existente si es posible."""
        # 1) Already have one
        if self._drone is not None:
            return self._drone

        # 2) Try grab from AppShell
        try:
            root = self.winfo_toplevel()
            d = getattr(root, "drone", None)
            if d is not None:
        ...
        # 3) Try reuse SensorStream's DroneClient (Sensores page)
        try:
            root = self.winfo_toplevel()
    ...
    # Compat: algunas acciones (y versiones previas) llamaban self._get_drone()
    # para obtener una conexión Olympe reutilizable.
    def _get_drone(self):
        return self._ensure_drone_for_control()

    def _set_stream_mode(self, drone, sensor: str):
        """ANAFI Thermal: cambia el modo térmico para afectar el RTSP."""
        try:
    ...
    def _capture_rgb_snapshot(self) -> str:
        """Captura RGB para acciones de misión.

        Importante: evitamos OpenCV->RTSP aquí porque en algunos entornos OpenCV
        abre el backend CAP_IMAGES y lanza el error:
        "can't find starting number ... rtsp://...".

        En su lugar, tomamos una **foto HD real** con Olympe y la descargamos
        vía media.download_media.
        """
    ...
    def _video_start(self, sensor: str, quality: str) -> str:
        ...
        # Nota: OpenCV/RTSP ya entrega un stream; acá usamos ffmpeg para grabación robusta.
        cmd = [
            "ffmpeg",
            "-hide_banner",
            "-loglevel",
        ...
        # quality placeholder (future: transcode). For now, keep copy.
        _ = quality
    ...
    def _resolve_speed_cmd(self):
        """Resolve MaxHorizontalSpeed command class once (avoid repeated module_loader errors)."""
        ...
        # Candidate locations across Olympe builds
        candidates = [
    ...
    def _try_set_gimbal_pitch(self, drone, pitch_deg: float):
        """Best-effort gimbal pitch using Olympe messages (ANAFI/Thermal)."""
        try:
            ...
            # Olympe gimbal.set_target uses degrees.
            exp = drone(
                set_target(
                    gimbal_id=0,
            ...
            # Keep silent to not break missions if gimbal API differs
            return

    def _move_by(self, drone, dx: float, dy: float, dz: float, dyaw_rad: float, speed_mps: float | None):
        """Move by using extended_move_by when speed is provided, else regular moveBy."""
        # Prefer extended_move_by since it lets us set max speeds per segment.
        if speed_mps is not None:
            try:
                from olympe.messages.move import extended_move_by  # type: ignore

                max_h = float(speed_mps)
                # Vertical speed: keep a sensible value; if dz==0 it won't matter.
                max_v = max(0.1, min(2.0, float(speed_mps)))
                # Yaw rotation speed in deg/s (Olympe docs).
                max_yaw = 60.0
