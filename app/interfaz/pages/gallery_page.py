# interfaz/pages/gallery_page.py
import os
import sys
import platform
...
# Raíz de media (misma convención que el resto de páginas)
try:
...
def _list_media_recursive(media_dir: str):
    """Busca imágenes/videos dentro de media_dir (y subcarpetas), excluye .part y archivos 0 B.
    Ordena por mtime descendente.
    """
    files = []
    for root, dirs, names in os.walk(media_dir):
        # Evita subcarpetas derivadas (en Thermal guardamos "mapped" y "matrices")
        # que NO deben mostrarse como elementos independientes de la galería.
...
class GalleryPage(ttk.Frame):
    """Galería: muestra imágenes y videos reales en ./media/rgb/{photos,videos}"""
