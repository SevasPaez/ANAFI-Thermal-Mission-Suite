...
class CameraControl:
    """
    Olympe + descarga real desde el servidor HTTP del Anafi.
    - Autodetecta 42.1 (directo) o 53.1 (SkyController 3).
    - Descarga atómica (.part → rename).
    - Espera a que el media quede indexado antes de bajar.
    """
    def __init__(self, drone=None, media_dir="./media", http_base=None):
        self.drone = drone
        self.media_dir = media_dir
        os.makedirs(self.media_dir, exist_ok=True)
        self.http_base = http_base  # si None, autodetect
    ...
    def _ensure_media_server(self):
        """
        Fija self.http_base probando /api/v1/media/medias en 42.1 y 53.1
        """
        if not self._sess:
            return False
        candidates = []
        ...
        # probar ambos si no se pasó explícito
    ...
    def _download_after_index(self, kind, timeout_s=12.0, poll_every=0.35):
        # Marca de tiempo “antes de disparar”
